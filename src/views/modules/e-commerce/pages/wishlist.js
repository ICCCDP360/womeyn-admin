            ...
            <Row className="row-cols-1 row-cols-md-2 row-cols-lg-2 row-cols-xl-3 mt-4 ">
                {
                    productList.map((item,index) => {
                        return (
                            <WishlistCard key={index} onRemove={() => handleRemove(index)} images={item.image} itemsName={item.name} ratings={item.rating} productPrice={item.price} productOffer={item.offer} productLiked={item.like}/>
                        )
                    })
                }
            </Row>
