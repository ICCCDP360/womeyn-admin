  ...
  useEffect(() => {
    getSellerServices()
      .then((res) => {
        ...
        let products = res.data.results.filter(
          (data) => data.categoryTypeId === 1
        );
        setProducts(products);
        let services = res.data.results.filter(
          (data) => data.categoryTypeId === 2
        );
        setServices(services);
        let both = res.data.results.filter((data) => data.categoryTypeId === 3);
        setBoth(both);
      })
      .catch((err) => console.log(err));
                      ...
                      <div className="d-flex flex-column">
                        <Row>
                          <Col sm="12">
                            ...
                            {sellerLimit[sellerLimit.length - 1] ===
                              sellers[sellers.length - 1] ||
                            sellerLimit[sellerLimit.length] === 0 ? (
                              <div className="loadMore-div">
                                <Button
                                  className="mt-5 loadMore-button"
                                  onClick={loadMoreSellers}
